...
public class Substring {
    ...
    private static int dfs(int[] nums, int index){
        if(index == nums.length)
            return 0;
        int nextSum = Math.max(dfs(nums, index + 1) + nums[index], nums[index]);
    ...
    private static int[] dfs(int index, int[] nums){
        ...
        int minValue = Math.min(nums[index] * values[0],
    ...
    /**
     * <h3>思路: 最长连续递增子数组</h3>
     * <h3>注: 这个题本身是采用贪心算法做的, 但是和子串问题很类似, 所以放在这里</h3>
     */
    ...
    private static int findLength3(int[] nums1, int[] nums2){
        // 注: 判断最大最小长度是为了确保两个数组相交的部分不会超过最小长度
        return nums1.length <= nums2.length ? findMaxLength(nums1, nums2) : findMaxLength(nums2, nums1);
    }

    private static int findMaxLength(int[] nums1, int[] nums2){
        int maxLength = 0;
        // 1. 第一个数组的左侧和第二个数组的右侧对齐
        for (int length = 1;length < nums1.length;length++){
            maxLength = Math.max(maxLength,
                    getMaxLength(nums1, 0, nums2, nums2.length - length, length));
        }
        // 2. 第一个数组和第二个数组中间对齐
        for (int index = nums2.length - nums1.length;index >= 0;index--){
            maxLength = Math.max(maxLength,
                    getMaxLength(nums1, 0, nums2, index, nums1.length));
        }
        // 3. 第一个数组的右侧和第二个数组的左侧对齐
        for (int length = nums1.length - 1;length >= 1;length--){
            maxLength = Math.max(maxLength,
                    getMaxLength(nums1, nums1.length - length, nums2, 0, length));
        }
        return maxLength;
    }

    /**
     * <h3>{@code length}: 两个数组相交的长度</h3>
     * <h3>注: 严格意义上来说, 这里并不能算滑动窗口</h3>
     */
    private static int getMaxLength(int[] nums1, int first, int[] nums2, int second, int length){
        int maxLength = 0, valid = 0;
        for (int index = 0;index < length;index++){
            valid = nums1[index + first] == nums2[index + second] ? valid + 1: 0;
            // 注: 只要碰见这种结算的, 都要考虑可能最后一次结算不了的情况
            maxLength = Math.max(maxLength, valid);
        }
